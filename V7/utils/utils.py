...
# Optional utilities: neither MOABB downloading nor MAML is used by the
# preprocessed-NPY LOSO experiment.  Keep those paths available when their
# dependencies are installed, without making them startup requirements.
try:
    import moabb
...
from utils.vision_dg_preprocess import subject_distribution_perturbation
...
def data_loader(Xs=None, Ys=None, Xt=None, Yt=None, args=None, domain_src=None):
    ...
    # V5 changes only source-domain training data. Prototypes and virtual donor
    # styles are computed from the source subjects after subject-wise EA; the
    # held-out target subject and its labels never enter this operation.
    if domain_src is None:
        raise ValueError(args.version + ' requires source domain IDs')
    Xs, records, _, _, _ = subject_distribution_perturbation(
        Xs, Ys, domain_src, args
    )
    applied = sum(record[-1] for record in records)
    print(
        'Source subject perturbation: {}/{} trials ({:.2f}%)'.format(
            applied, len(records), 100.0 * applied / max(len(records), 1)
        )
    )
